...
- **AI Coach 3 tầng**: (a) Hỏi Coach — **2 chế độ**: ⚡ Nhanh (số liệu, offline, tức thì) + 🧠 AI 7B (chat tự do với LLM trên máy, chỉ desktop); (b) Coach offline — LLM 7B tự viết phân tích chuyên sâu (desktop); (c) Hỏi Claude qua mạng (chỉ khi bấm). (a-AI7B và (b) dùng CHUNG engine 7B singleton.
...
1. **Thử Coach offline (Qwen 7B) thật trên Mac 16GB** (bấm "Phân tích chuyên sâu") — đã nâng 3B→7B nhưng CHƯA chạy thử thật. Cần xem: (a) 16GB có tải/chạy nổi 7B không, mỗi câu chậm cỡ nào; nếu ì/lỗi → nút "Thử mô hình nhỏ hơn" (3B). (b) 7B còn trôi tiếng Trung / mượn số ví dụ vàng / từ nhân-quả nữa không (kỳ vọng ít hơn 3B nhiều). Nếu 7B quá nặng cho 16GB → cân nhắc hạ mặc định về 3B hoặc thử `gemma-2-2b-it`. Nếu muốn khôn hơn nữa & máy mạnh → `gemma-2-9b-it` (~6.3GB).
