...
## `StatsDashboard.jsx` (294 dòng, `src/components/`) + `StatsJournal.jsx` · `StatsNotes.jsx` · `engine/statsAnswers.js`
**Nhiệm vụ**: màn Thống kê TRẢ LỜI ba câu (ADR-071, 2026-09-06): *khá lên không* (tuần này so CÙNG QUÃNG tuần trước) · *mạnh nhất khi nào* (giờ · độ dài · loại việc, mỗi dòng kèm cỡ mẫu) · *làm gì tiếp* (một nút «Bắt đầu N phút · loại» nhảy sang màn Tập trung), rồi dải «Điều đáng chú ý» (`statsInsights.js`), rồi sổ tra cứu Nhật ký · Ghi chú GẤP dưới cùng. **Phụ thuộc**: `engine/statsAnswers.js` (thuần — chỉ ghép `coach/coachIntel.js` `buildFocusProfile`/`recommendNextSession` và `gameMath.js`; không chế công thức mới), `time.vietnamHistoryTimeOpts` (bộ getter giờ VN dùng chung với AI Coach), `statsFormatters.js` · `statsTheme.js` (định dạng + màu cho hai cuốn sổ), `App.jsx` truyền `onNavigate`. **Luật**: một con số không có mẫu số thì không phải mục tiêu — mọi tỉ lệ đi kèm «trên N phiên»; `statsNavClarity.test.js` đọc mã nguồn canh thứ tự ba câu, sổ gấp mặc định, không cuộn ngang, nút có dây thật. Trước 2026-09-06 file này 3.792 dòng với 5 tab con · 6 kỳ · biểu đồ · bản đồ nhiệt — đã xoá (`statsPeriod.js` · `statsFocus.js` cũng xoá).
...
## Module nào để sau
`src/components/StatsDashboard.jsx` (lớn, chủ yếu là hiển thị, không ảnh hưởng game logic — an toàn để hiểu muộn hơn); `electron/` (chỉ cần hiểu khi thực sự sửa tính năng tray Mac); các file `BuildScreen.jsx`/`RelicInventory.jsx`/`SkillTree.jsx` (UI cho các hệ thống nâng cấp — từ ADR-069 công trình chỉ tốn PHIÊN, có thể đọc theo nhu cầu cụ thể thay vì đọc trước toàn bộ).
