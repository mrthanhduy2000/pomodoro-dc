 ...
 *
 * ⚠️ TẦNG TRỆT (cửa ra vào + đặc trưng mặt phố) **KHÔNG còn ở file này** — nó đã dọn sang
 * `groundFloorStyle.js` ngày 2026-08-18 (ADR-029), đúng khuôn "bảng ↔ hình" mà `floraStyle.js`,
 * `streetStyle.js` và `horizon.js` đã dùng. Lý lẽ cũ giữ nó ở đây (*"câu trả lời `country` phải
 * nằm trong tầm mắt"*) **đã bị bác**: ràng buộc ấy xưa nay do một BÀI TEST giữ, không do khoảng
 * cách trên màn hình — và bài test ấy chạy y nguyên sau khi bảng dọn đi.
 */
...
export function getEraStyle(era) {
  return ERA_STYLES[normalizeEraKey(era)];
}

/**
 * Số kỷ đã chuẩn hoá — "kỷ lạ thì tra ra kỷ nào".
 *
 * ⚠️ XUẤT RA (2026-08-18) để `groundFloorStyle.js` (và mọi bảng 15 kỷ sau này, kể cả bảng mái của
 * Phase 11) hỏi ĐÚNG phép chuẩn hoá này thay vì chép lại `Math.round` + `?? DEFAULT_ERA`. Hai công
 * thức tương đương trên giấy gần như luôn lệch nhau ở biên — dự án đã trả giá cho đúng chuyện đó
 * ở Phase 3Y (`horizonHue < 60` vs `warm(h)`), và luật rút ra là **một luật chỉ được có một công
 * thức**. Bảng nào cũng tra theo cùng một khoá thì kỷ 2,4 hay kỷ 99 đều rơi về cùng một chỗ ở mọi
 * bảng, không có chuyện mái tra ra kỷ này mà cửa tra ra kỷ khác.
 */
export function normalizeEraKey(era) {
  const key = Number.isFinite(era) ? Math.round(era) : DEFAULT_ERA;
  return ERA_STYLES[key] ? key : DEFAULT_ERA;
}
