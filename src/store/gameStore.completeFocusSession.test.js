...
// ═══════════════════════════════════════════════════════════════════════════════
// 12) Streak — nối chuỗi qua ngày (tài sản chuỗi không mất khi làm đều)
// ═══════════════════════════════════════════════════════════════════════════════
test('completeFocusSession: hôm qua có chuỗi 5 ⇒ phiên hôm nay nối thành 6', () => {
  resetStore();
  const yesterday = localDateStr(Date.now() - 86_400_000);
  useGameStore.setState((st) => ({
    streak: { ...st.streak, currentStreak: 5, longestStreak: 5, lastActiveDate: yesterday },
  }));
  withRandom(NO_EVENT, () => useGameStore.getState().completeFocusSession(25));
  const s = useGameStore.getState();
  assert.equal(s.streak.currentStreak, 6);
  assert.equal(s.streak.longestStreak, 6);
  assert.equal(s.streak.lastActiveDate, localDateStr());
});

// ═══════════════════════════════════════════════════════════════════════════════
// 13) Streak — bỏ cách nhiều ngày (không lá chắn) ⇒ chuỗi tính lại từ 1
// ═══════════════════════════════════════════════════════════════════════════════
test('completeFocusSession: bỏ 3 ngày (không lá chắn) ⇒ chuỗi reset về 1', () => {
  resetStore();
  const threeDaysAgo = localDateStr(Date.now() - 3 * 86_400_000);
  useGameStore.setState((st) => ({
    streak: { ...st.streak, currentStreak: 5, lastActiveDate: threeDaysAgo },
  }));
  withRandom(NO_EVENT, () => useGameStore.getState().completeFocusSession(25));
  const s = useGameStore.getState();
  assert.equal(s.streak.currentStreak, 1);
