# ARCHITECTURE DECISIONS — ADR-001 … ADR-50, frozen 2026-09-06 (ADR-075)

> Full text of **50 older ADRs**, moved verbatim out of the active
> `ARCHITECTURE_DECISIONS.md`. **Nothing was deleted or rewritten.** The project rule *"never delete
> an old ADR, even when a later decision reverses it, so the history of reasoning is not lost"* is
> intact — these records still exist, in full, one `grep` away.
>
> **Why:** the active file had reached 392,634 chars ≈ 227,878 tokens = **114% of a 200k context
> window**. A decision made years of phases ago is consulted rarely; a decision from the last few
> weeks is consulted constantly. Splitting on that boundary keeps the hot set cheap without losing
> the cold set.
>
> **How to find one:** `grep -n '^## ADR-0NN' docs/archive/ADR_ARCHIVE_001-050.md` or
> `node scripts/doc-budget.mjs --map docs/archive/ADR_ARCHIVE_001-050.md` for the index without reading the file.
> The active file keeps a one-line index for every ADR below.
