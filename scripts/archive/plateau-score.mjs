...
export function hoSoDocTheoBanKinh(mat, { buoc = 0.5, toi = 12 } = {}) {
  ...
  for (let r = buoc / 2; r <= toi; r += buoc) {
    ...
    for (let i = 0; i < GOC; i += 1) {
      ...
      const h = mat(x, z);
      tongCao += h;
      if (h > caoMax) caoMax = h;
    }
    vanh.push({ r, doc: tongDoc / GOC, cao: tongCao / GOC, caoMax });
  }
  return vanh;
}
...
/**
 * ĐỘ NHÔ CỦA BỆ — chênh cao trung bình giữa vùng trong và vùng ngoài, đo bằng ĐƠN VỊ CAO ĐỘ.
 *
 * ⚠️ VÌ SAO PHẢI CÓ CỘT NÀY BÊN CẠNH CHỈ SỐ BỆ, VÀ VÌ SAO NÓ KHÔNG THAY THẾ ĐƯỢC CHỈ SỐ ẤY.
 * "Bệ" là một từ ghép hai điều kiện: **phẳng** VÀ **nhô lên**. Chỉ số bệ (tỉ số dốc) chỉ đo được
 * vế thứ nhất, nên nó không phân biệt nổi *"một mặt bàn nổi giữa sàn"* với *"một kỷ TỰ KHAI mình
 * phẳng tuyệt đối, nằm giữa đồng"* — kỷ 14 khai thẳng `terraces: 1, relief: 0` (Marina Bay là đất
 * lấn biển, do người san) nên `docTrong` của nó là 0,0000 **theo thiết kế**, và mọi tỉ số chia cho
 * 0 đều ra một con số to. Cột này trả lời đúng vế còn lại: *đất trong lưới có NHÔ LÊN không?*
 *
 * Kèm `caoNhatNgoai`: nếu đồng bằng ở đâu đó **cao hơn** nền phố thì không thể có mặt bàn nào cả —
 * mặt bàn là thứ mà mọi hướng nhìn đều thấy nó nổi trên nền. Một `true` ở cột ấy là bằng chứng
 * mạnh hơn mọi tỉ số.
 */
export function doNhoBe(vanh, { trong = [0, 5], ngoai = [6, 9] } = {}) {
  const trg = vanh.filter((v) => v.r >= trong[0] && v.r <= trong[1]);
  const ngo = vanh.filter((v) => v.r >= ngoai[0] && v.r <= ngoai[1]);
  if (!trg.length || !ngo.length) return { nho: NaN, caoNhatNgoai: NaN, nenPho: NaN };
  const nenPho = trg.reduce((s, v) => s + v.cao, 0) / trg.length;
  const ngoaiTb = ngo.reduce((s, v) => s + v.cao, 0) / ngo.length;
  const caoNhatNgoai = Math.max(...ngo.map((v) => v.caoMax ?? v.cao));
  return { nho: nenPho - ngoaiTb, caoNhatNgoai, nenPho };
}
...
for (const era of eras) {
  ...
  const k = chiSoBe(vanh);
  const nho = doNhoBe(vanh);
  bang.push({ era, vanh, ...k, ...nho });
  if (arg.hoso) {
...
console.log('\nkỷ\tchỉ số bệ\tdốc trong\tdốc ngoài\tr nhảy\tđộ NHÔ\tđồng cao hơn phố?');
for (const b of bang) {
  console.log(`${b.era}\t${b.chiSo.toFixed(2).padStart(8)}\t${b.docTrong.toFixed(4).padStart(9)}`
    + `\t${b.docNgoai.toFixed(4).padStart(9)}\t${b.rNhay.toFixed(2).padStart(5)}`
    + `\t${b.nho.toFixed(3).padStart(6)}\t${b.caoNhatNgoai > b.nenPho ? 'CÓ' : '— không'}`
    + `  (đỉnh đồng ${b.caoNhatNgoai.toFixed(3)} vs nền phố ${b.nenPho.toFixed(3)})`);
}
if (bang.length > 1) {
  ...
  const nhos = bang.map((b) => b.nho);
  const soCaoHon = bang.filter((b) => b.caoNhatNgoai > b.nenPho).length;
  console.log(`ĐỘ NHÔ trung bình: ${trungBinh(nhos).toFixed(3)}`
    + `  ·  cao nhất: kỷ ${bang[nhos.indexOf(Math.max(...nhos))].era} = ${Math.max(...nhos).toFixed(3)}`);
  console.log(`SỐ KỶ mà đồng bằng có chỗ CAO HƠN nền phố (⇒ không thể là mặt bàn): ${soCaoHon}/15`);
