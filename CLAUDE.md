...
- **Thẻ AI Coach (briefing)** — câu động viên ngắn 1 dòng, engine luật `src/engine/coachVoice.js` + `src/hooks/useCoachVoice.js`. ⚠️ **(2026-06-21) Đã CHỐT MỘT giọng cố định** = hằng `FIXED_COACH_PERSONALITY='zen'` trong `useCoachVoice.js`; **đã bỏ bộ chọn 3 nút** strict/zen/buddy khỏi `CoachCard.jsx`/`FocusRail.jsx`/`FocusCoachMobile.jsx`. Engine `coachVoice.js` vẫn còn cả 3 giọng (test sim 4104/4104) nhưng chỉ `zen` được nối; muốn đổi giọng → sửa hằng đó. `settingsStore.coachPersonality` thành orphan (giữ cho dữ liệu cũ, không UI/hook nào đọc). KHÔNG liên quan Coach offline (b).
