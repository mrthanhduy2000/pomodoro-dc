...
- Luôn `npm test` trước khi commit (hiện 147 bài) + `npm run build` (xác nhận precache không có webllm). Công cụ mắt-soi bảng số liệu Qwen nhận: `node --import ./scripts/register-esm-loader.mjs scripts/coach-sample.mjs` (dựng lịch sử mẫu ~24 giờ + in `buildAnalystContext`).
