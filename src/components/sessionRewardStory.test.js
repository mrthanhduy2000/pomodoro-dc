...
test('thứ tự câu chuyện: xp → NHỊP (chuỗi + hôm nay) → nhiệm vụ → lên cấp → kỷ mới', () => {
  ...
  // ⚠️ ROUND 46 (ADR-086): «hôm nay» không còn là một thẻ riêng — nó là nửa dưới của thẻ «chuỗi».
...
// ⚠️ ROUND 46: cùng phép tính ấy, nay đọc ở `streak.today`. Gộp thẻ KHÔNG được đổi một con số nào —
...
test('thứ tự đầy đủ: xp → công trình → NHỊP → nhiệm vụ → BƯỚC TUẦN → thử thách → lên cấp → bậc → di vật → LÊN BẬC → kỷ', () => {
  ...
  // ⚠️ ROUND 46 (ADR-086): `today` là nửa dưới của «streak»; `rank`+`relic`+`evolve` là ba HÀNG
...
// ⚠️ ROUND 46: nay là một HÀNG trong «treasure». Ba thẻ gộp làm một được phép tốn một cái thẻ —
